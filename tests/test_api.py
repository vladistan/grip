...
def test_app(monkeypatch, tmpdir):
    ...
    with GitHubRequestsMock() as responses:
        # Use empty tuple to prevent loading auth from settings
        assert Grip(zero_path, auth=(), assets=assets).render() == zero_output
        assert Grip(zero_path, auth=(), assets=assets).render('/') == zero_output
        assert Grip(zero_path, auth=(), assets=assets).render('/x/../') == zero_output
        with Grip(zero_path, auth=(), assets=assets).test_client() as client:
            assert client.get('/').data.decode('utf-8') == zero_output
        assert len(responses.calls) == 4

    with GitHubRequestsMock() as responses:
        app = Grip(gfm_test_path, auth=(), assets=assets)
